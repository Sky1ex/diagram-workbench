/**
 * Генерирует src/data/graph-document-5k.json:
 * — ~5000 узлов суммарно по всем графам;
 * — ~5000 рёбер суммарно;
 * — вложенные подграфы;
 * — не более MAX_NODES_PER_GRAPH узлов в каждом GraphDefinition (включая main);
 * — не более MAX_NODES_PER_DEPTH узлов на один depth внутри графа.
 *
 * Запуск: node scripts/generate-graph-document-5k.mjs
 */
...
const OUT = join(__dirname, '../src/data/graph-document-5k.json')
